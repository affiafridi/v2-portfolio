...
/* Image ends at 8vw padding each side — all sections match this width */
const SIDE = '8vw'
...
export default function BlogPostPage() {
  ...
  const next    = posts[(postIdx + 1) % posts.length]
  ...
  /* ── Featured image — grows on scroll, bounded, reverses on up ─── */
  useEffect(() => {
    ...
    tl.fromTo(inner,
      { width: 300,    borderRadius: '8px' },
      { width: '84vw', borderRadius: '8px', ease: 'none' },
    )

    return () => { tl.kill() }
  }, [slug])

  /* ── Article body scroll animations — Brief style ──────────────── */
  useEffect(() => {
  ...
  if (!post) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: CREAM }}>
        <Link href="/blog" style={{ fontSize: '13px', color: ACC, fontWeight: 600 }}>← Back to Blog</Link>
      </div>
    )
  }
  ...
  return (
    <div ref={pageRef} style={{ background: CREAM }}>
      ...
      {/* ══ HERO — centered, compact, image butts right up ═══════════ */}
      <section style={{
        display:        'flex',
        flexDirection:  'column',
        alignItems:     'center',
        textAlign:      'center',
        paddingTop:     'clamp(100px, 22vh, 160px)',
        paddingBottom:  0,
        paddingLeft:    'clamp(32px, 6.5vw, 96px)',
        paddingRight:   'clamp(32px, 6.5vw, 96px)',
      }}>

        {/* ← Blog link + number — replaces the category label */}
        <div className="bpg-label" style={{ display: 'flex', alignItems: 'center', gap: '14px', justifyContent: 'center', marginBottom: 'clamp(18px, 2.5vw, 28px)' }}>
          <Link
            ...
            onMouseLeave={e => { e.currentTarget.style.color = `${INK}42`; setCursorType('default') }}
          >
            <svg width="12" height="9" viewBox="0 0 12 9" fill="none" aria-hidden>
              <path d="M11 4.5H1M5 1L1 4.5 5 8" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            Blog
          </Link>
        ...
        {/* Title — no bottom margin, image follows directly */}
        <h1
          className="bpg-title"
          style={{
      ...
      {/* ══ FEATURED IMAGE — grows on scroll ════════════════════════ */}
      <div ref={imgWrapRef} style={{ paddingTop: '32px', display: 'flex', justifyContent: 'center' }}>
        <div
          ref={imgInnerRef}
          style={{ overflow: 'hidden', aspectRatio: '16 / 9', flexShrink: 0 }}
        >
      ...
      {/* ══ ARTICLE BODY — Brief section style ══════════════════════ */}
      <section className="bpg-body" style={{
        background: CREAM,
        padding: `clamp(60px, 8vw, 120px) ${SIDE}`,
      }}>
        ...
        <div style={{
          ...
          <div className="bpg-brief-line" style={{
        ...
        {/* ── Large word-by-word excerpt reveal ───────────────────── */}
        <p style={{
          ...
          {post.excerpt.split(' ').map((word, wi) => (
            <span key={wi} className="bpg-brief-word" style={{
        ...
        {/* ── Divider ─────────────────────────────────────────────── */}
        <div className="bpg-brief-divider" style={{
          height: 0, borderTop: `1px solid ${INK}`,
          marginBottom: 'clamp(40px, 5vw, 72px)',
        }} />

        {/* ── 2-col: article content + sidebar ────────────────────── */}
        <div style={{
          display: 'grid',
          ...
          <article className="bpg-sidebar">
            ...
            <h2 className="bpg-block" style={{ fontSize: 'clamp(20px, 2.2vw, 32px)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.10, color: INK, margin: '0 0 clamp(14px, 1.8vw, 22px)' }}>
              The thinking behind it
            </h2>

            <p className="bpg-block" style={{ fontSize: 'clamp(14px, 1.15vw, 17px)', lineHeight: 1.88, color: `${INK}62`, margin: '0 0 clamp(16px, 1.8vw, 24px)', textAlign: 'justify' }}>
              Most problems in design and development come from skipping the uncomfortable questions early on. The brief looks complete, the timeline looks feasible, and then somewhere between week two and the first client review, something quietly falls apart.
            </p>

            <p className="bpg-block" style={{ fontSize: 'clamp(14px, 1.15vw, 17px)', lineHeight: 1.88, color: `${INK}62`, margin: '0 0 clamp(28px, 4vw, 52px)', textAlign: 'justify' }}>
              The solution is rarely more process. It is usually better judgment — knowing which constraints are real and which are negotiable, and knowing when to push back versus build and show rather than tell.
            </p>
            ...
            <p className="bpg-block" style={{ fontSize: 'clamp(14px, 1.15vw, 17px)', lineHeight: 1.88, color: `${INK}62`, margin: '0 0 clamp(16px, 1.8vw, 24px)', textAlign: 'justify' }}>
              When you stop trying to eliminate ambiguity and start working with it, everything moves faster. Decisions that used to require three rounds of approval get made in the room.
            </p>

            <div className="bpg-block" style={{ borderLeft: `2px solid ${ACC}`, paddingLeft: '18px' }}>
              <p style={{ fontSize: 'clamp(12px, 0.95vw, 14px)', lineHeight: 1.72, color: `${INK}38`, margin: 0, fontStyle: 'italic' }}>
                Full article content coming soon — this post is a placeholder layout demonstration.
              </p>
            </div>
          </article>
      ...
      {/* ══ NEXT ARTICLE — same horizontal width as image at full size */}
      <Link
        href={`/blog/${next.slug}`}
        className="bpg-next"
        style={{
      ...
      >
        ...
        <div style={{ fontSize: 'clamp(11px, 1vw, 14px)', fontWeight: 500, color: `${CREAM}30`, marginTop: '16px', letterSpacing: '0.04em' }}>
          {next.readTime} read · {next.category}
        </div>
      </Link>

      <style>{`
        .bpg-next:hover .bpg-next-title { color: ${ACC}; }
        .bpg-next:hover .bpg-next-arrow {
